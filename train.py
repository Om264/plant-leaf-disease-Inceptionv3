"""Fine-tune timm Inception v3 on a plant-leaf disease dataset, then evaluate on the test set.
...
def parse_args(argv=None):
    ...
    ap.add_argument("--model", default="inception_v3",
                    help="timm model name, e.g. inception_v3, inception_resnet_v2")
    ap.add_argument("--no_pretrained", action="store_true", help="random init (offline / tests)")
    ap.add_argument("--no_aux_logits", action="store_true",
                    help="disable the auxiliary classifier head (default: on, per the original recipe)")
    ap.add_argument("--aux_weight", type=float, default=0.4, help="weight of the aux-head loss")
    ap.add_argument("--epochs", type=int, default=30)
    ap.add_argument("--batch_size", type=int, default=32, help="Inception v3 at 299px is memory-hungry; lower this if you run out of GPU memory")
    ap.add_argument("--lr", type=float, default=1e-3)
    ap.add_argument("--weight_decay", type=float, default=1e-2)
    ap.add_argument("--dropout", type=float, default=0.2)
    ap.add_argument("--label_smoothing", type=float, default=0.1)
    ap.add_argument("--img_size", type=int, default=299, help="Inception v3 native size is 299")
...
def main(argv=None):
    ...
    # ---- model ---------------------------------------------------------------------
    use_aux = not args.no_aux_logits
    model = create_model(args.model, num_classes, pretrained=not args.no_pretrained,
                         dropout=args.dropout, aux_logits=use_aux).to(device)
    n_params = sum(p.numel() for p in model.parameters())
    print(f"Model: {args.model} | aux_logits: {use_aux} | parameters: {n_params/1e6:.2f} M")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        tr_loss, tr_acc = train_one_epoch(model, train_loader, criterion, optimizer,
                                          scaler, device, use_amp, aux_weight=args.aux_weight)
        scheduler.step()
