...
from plantdisease.model import forward_logits, load_checkpoint
...
class GradCAM:
    ...
    def __call__(self, x, class_idx=None):
        ...
        with torch.enable_grad():
            logits = forward_logits(self.model, x)  # discard aux head, timm returns it even in eval()
...
def main(argv=None):
    ...
    ap.add_argument("--target_layer", default=None,
                    help="module name, default: last Inception block (Mixed_7c)")
    ...
    layer = model.get_submodule(args.target_layer) if args.target_layer else model.Mixed_7c
